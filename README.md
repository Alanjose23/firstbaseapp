...
The app deploys to **[Render](https://render.com)** as a single web service: the Express
server serves the GraphQL API *and* the built React client from the same origin. A
[`render.yaml`](./render.yaml) blueprint is included, and pushes to `main` auto-deploy.

A GitHub Actions workflow ([`.github/workflows/ci.yml`](./.github/workflows/ci.yml))
runs on every push and pull request to `main`, installing dependencies and building the
client so broken builds are caught before they reach Render.

### 1. Set up the database (MongoDB Atlas)

1. Create a free cluster at [mongodb.com/atlas](https://www.mongodb.com/atlas).
2. Add a database user and allow network access from anywhere (`0.0.0.0/0`) so Render can connect.
3. Copy the connection string, e.g.
   `mongodb+srv://<user>:<pass>@<cluster>.mongodb.net/firstbaseapp?retryWrites=true&w=majority`.

### 2. Deploy to Render

1. Push this repo to GitHub.
2. In Render, choose **New → Blueprint** and point it at this repository. Render reads
...
3. When prompted, set the environment variables:
   ...
   | `JWT_SECRET` | Auto-generated by Render (`generateValue`) — no action needed |
   | `NODE_ENV` | Pre-set to `production` |
   | `CLIENT_URL` | Optional — leave blank for this single-origin setup (the server falls back to Render's own URL) |

4. Render runs `npm install && npm run build`, then `npm start`. The service comes up
   on the port Render provides via `PORT`.

> **Note:** The build installs `devDependencies` (Vite et al.) via the `--include=dev`
> flags in the root `install` script, so the client build works even with
> `NODE_ENV=production`.
